...
We were thinking about how Mojang might patch Gaslight v5, and the obvious solution was to make the modified check much more sensitive. This is very similar to v5, but we take an alert fatigue approach instead. The first change is inserting a zero width spacer into every message, this forces the game to recognise it as modified, but most players would dismiss this as typical server jank. Then we add a custom hover event to the message, so when they hover it, they see the expected modified message box, except we just add one that looks identical with a faked original message. The result is heavily obfuscating the swapped messages. Every message shows as modified, and when you hover it, you see what you saw in chat. This makes it very challenging for an unsuspecting player to actually notice the swap. In theory, a player could notice this by hovering the tiny bar to the left instead of the message, or the icon to the right, but that icon is covered by our hover event. Our belief is that the vast majority of players do not know that you need to hover the bar on the left. The exploit path here is largely similar to v5 or v3, except there's no requirement to extend the message like in v5.
